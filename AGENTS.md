...
**Dead-Run Watchdog & Worktree Orphan Reaping (`internal/daemon/watchdog.go`, `worktree_reap*.go`)**

- While the daemon is alive, the watchdog is the ONLY actor on run liveness: `step_quiet_warning` stays display-only, `ci_timeout` owns the CI monitor's idle lifetime (the monitor dedupes unchanged poll logs, so CI is exempt from the stall kill), and parked `awaiting_agent` runs are exempt by design. Deadness reasons carry `types.RunDeadReasonPrefix` (`dead run: `) and flow through the executor's cancel-cause path to a terminal `failed`; `axi status` adds a client-side `liveness: ok|stalled|dead` for active runs (covers a stopped daemon, which the watchdog cannot). Keep those three ownerships when touching any of them.
- Two liveness-evidence invariants keep the stall kill honest: configured `commands.*` invocations are captured buffered, so `runStepShellCommand` heartbeats step activity while the daemon waits on the live command (`startConfiguredCommandHeartbeat`; without it a 1h+ healthy test suite looks dead), and a row inside `startRun` setup is owned via `RunManager.settingUp` so a long checkout/fetch is never mistaken for an orphaned row.
- Run worktree removal must go through `removeRunWorktree`, which first reaps processes anchored to the worktree (cwd inside it via /proc, or worktree path in argv via ps): grandchildren that move into a new process group escape the shellenv boundary and both leak and defeat `git worktree remove --force` by re-creating files mid-delete. Windows reaping is intentionally a no-op.
- Regressions: `internal/daemon/watchdog_test.go`, `internal/daemon/worktree_reap_unix_test.go`, `TestAxiStatusLiveness*` (`internal/cli/axi_test.go`), `TestLoadGlobal_StepStallTimeout`, `TestRunStepShellCommand_HeartbeatsStepActivityWhileCommandRuns`.
