...
Who owns broad regression follows [`ci_mode`](/no-mistakes/reference/global-config/#ci_mode). Under `ci_mode: github` local Test is never a repository-wide regression-suite substitute: broad regression is owned by remote CI and remains mandatory before a PR is ready. Under `ci_mode: local` no forge checks run for the change, so the evidence turn runs the repository's complete regression suite itself, after the targeted scenarios; fix rounds stay focused in both modes, so the suite is paid once per Test step rather than once per repair.
...
- Bounds those agent turns with [`test_agent_timeout`](/no-mistakes/reference/global-config/#test_agent_timeout): each evidence-gathering or Test-repair invocation gets its own budget, and an expired budget cancels the agent and fails the step with a timeout diagnostic rather than leaving the run active indefinitely. Under `ci_mode: local` the evidence turn's complete-suite run shares that same budget, so a repository whose suite outlasts `test_agent_timeout` needs a larger value.
- The agent may use an existing automated test only when it drives the scenario end to end. A broad suite run is never live scenario evidence: when the agent cannot establish the intent with a targeted check or manual verification, it reports what is missing rather than treating broad regression or a unit test as live evidence. Under `ci_mode: github` it must not run the complete repository suite at all; under `ci_mode: local` it runs that suite in addition to the scenarios and reports a suite failure as a finding.
